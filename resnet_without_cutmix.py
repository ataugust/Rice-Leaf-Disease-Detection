...
NUM_EPOCHS_FROZEN = 10       # Phase 1 epochs
NUM_EPOCHS_FINETUNE = 20     # Phase 2 epochs
...
# CHANGED: Save under a new name so it doesn't overwrite your 96% model
CHECKPOINT = "resnet_rice_best_no_cutmix.pth" 
...
def run_epoch(loader, model, criterion, optimizer=None, train=True, scaler=None):
    ...
    for inputs, labels in loop:
        ...
        # --- CUTMIX LOGIC ---
        # CHANGED: CutMix DISABLED per guide's request for ablation study
        apply_cutmix = False
...
if __name__ == '__main__':
    ...
    # --- PHASE 1 ---
    print("\nPhase 1: Training classifier head only (NO CUTMIX)")
    ...
    # --- PHASE 2 ---
    print("\nPhase 2: Fine-tuning layer4 and fc (NO CUTMIX)")
    ...
    # CHANGED: Graph Title and Filename
    cm = confusion_matrix(all_labels, all_preds)
    plt.figure(figsize=(10, 8))
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=class_names, yticklabels=class_names)
    plt.title('Confusion Matrix - ResNet50 (30 Epochs, No CutMix)', fontsize=14)
    plt.ylabel('Actual Label', fontsize=12)
    plt.xlabel('Predicted Label', fontsize=12)
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    plt.savefig('confusion_matrix_no_cutmix.png', dpi=300) 
    plt.close()
    ...
    plt.title('Model Performance Metrics by Disease Class (No CutMix)', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig('metrics_barchart_no_cutmix.png', dpi=300)
    plt.close()

    print("Graphs saved successfully as 'confusion_matrix_no_cutmix.png' and 'metrics_barchart_no_cutmix.png'!")